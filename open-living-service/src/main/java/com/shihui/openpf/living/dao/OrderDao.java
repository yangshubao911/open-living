...
public class OrderDao extends AbstractDao<Order> {
	...
	public List<Order> queryTopN(int count) {
		return this.queryForList( "SELECT * FROM `living_order` WHERE order_status = ? ORDER BY update_time DESC  LIMIT ?", new Object[]{OrderStatusEnum.OrderHadReceived.getValue(),count});
	}

	public List<Order> queryOrderUnStockOut() {
//		return this.queryForList( "SELECT * FROM `living_order` WHERE order_status = ? ORDER BY update_time ASC", new Object[]{OrderStatusEnum.OrderUnStockOut.getValue()});
			return this.queryForList( "SELECT a.* FROM `living_order` AS a INNER JOIN `bill` AS b ON a.order_id = b.order_id WHERE a.order_status = ? AND b.bill_status != ? AND b.bill_status != ?",
	...
	public List<Order> queryOrder(Order order, String startTime, String endTime, Integer page, Integer size) {
		StringBuilder sql = new StringBuilder(
				"select a.*,b.request_id,c.service_start_time from (`living_order`a left join `request` b on a.order_id = b.order_id) left join `contact` as c on a.order_id = c.order_id where 1 = 1 ");
	...
	public int countQueryOrder(Order order, String startTime, String endTime) {
		StringBuilder sql = new StringBuilder("select count(*) from `living_order` where 1 = 1 ");
		Field[] fields = Order.class.getDeclaredFields();
		try {
			ArrayList<Object> valus = new ArrayList<Object>();
			for (Field field : fields) {
	...
	public Order queryOrder(long orderId) {
		String sql = "select * from `living_order` where order_id = ?";
		try {
	...
	public int countUnusual() {
		return super.queryCount("SELECT count(*) FROM `living_order` WHERE order_status = ? AND update_time < DATE_ADD(NOW(), INTERVAL -24*3 HOUR) ", 
	...
	public List<Order> queryUnusual() {
		return super.queryForList("SELECT * FROM `living_order` WHERE order_status = ? AND update_time < DATE_ADD(NOW(), INTERVAL -24*3 HOUR) ORDER BY update_time DESC ", 
				OrderStatusEnum.OrderUnStockOut.getValue());
	}
	...
	public int countOrders(long userId, int serviceId, String deviceId) {
		String sql = "select count(*) from `living_order` where (user_id = ? or device_id=?) and service_id = ? and order_status in ("
	...
	public int updateOrderStatus(long orderId, int status) {
		return this.jdbcTemplate.update("UPDATE `living_order` SET order_status = ?, update_time = ? WHERE order_id = ?", new Object[]{status, new Date(), orderId});
	}
	
	public boolean isFirstBill(int userId) {
		return this.queryCount("SELECT user_id FROM `living_order` WHERE user_id = ? LIMIT 1", new Object[]{userId}) > 0;
	}
}
